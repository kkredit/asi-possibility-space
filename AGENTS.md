...
5. `cachedOutcomes = baseCells.flatMap(expandTakeoff)` → **432 cells**, spanning
   all 7 factors. A test asserts this count and that every enumerated scenario is
   reasoned.
...
- Coverage today:
  - [`src/engine/engine.test.ts`](src/engine/engine.test.ts) — enumeration,
    ...
    dataset integrity (no duplicate keys, valid ids, values in `[-1,1]`, all 432
    cells authored).
...
- **New engine math MUST get a test.** The engine is the trustworthy core; keep it
  ...
  (sums to 1, cells in range, 432 authored).
...
- Don't hand-author all 432 cached cells — use `cell` / `failsTable` /
  `expandTakeoff`.
